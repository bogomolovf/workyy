...
export function analyzeDataColumns(result: SqlResult): ColumnAnalysis[] {
  ...
  for (let colIdx = 0; colIdx < result.columns.length; colIdx++) {
    ...
    analyses.push({
      ...
      sampleValues: Array.from(uniqueValues).slice(0, 5).map((v) => {
...
export function validateFieldMapping(result: SqlResult, config: PlotConfig): { valid: boolean; message?: string } {
  ...
  if (config.mapping.y) {
    ...
    for (const yField of yFields) {
      ...
      const numericRequired = ["bar", "bar-horizontal", "line", "area", "scatter", "boxplot", "radar"];
      ...
      if (yCol && config.chartType === "histogram" && yCol.type !== "numeric" && yCol.type !== "temporal") {
        return { valid: false, message: `Y field "${yField}" must be numeric or temporal for histogram` };
      }
    }
  ...
  if (config.chartType === "histogram") {
    ...
    if (config.mapping.x) {
      const xCol = columnMap.get(config.mapping.x);
      if (xCol && xCol.type !== "numeric" && xCol.type !== "temporal") {
        return { valid: false, message: `X field "${config.mapping.x}" must be numeric or temporal for histogram` };
      }
    }
    if (config.mapping.y) {
      const yCol = columnMap.get(config.mapping.y);
      if (yCol && yCol.type !== "numeric" && yCol.type !== "temporal") {
        return { valid: false, message: `Y field "${config.mapping.y}" must be numeric or temporal for histogram` };
      }
    }
  }
  ...
  if (config.chartType === "treemap") {
    ...
    const hasYField = config.mapping.y && (Array.isArray(config.mapping.y) ? config.mapping.y.length > 0 : true);
  ...
  if (config.mapping.color) {
    ...
    if (colorCol && colorCol.type !== "categorical" && config.chartType !== "scatter" && config.chartType !== "heatmap") {
  ...
  if (config.mapping.facet) {
    ...
    if (facetCol) {
      ...
      if (uniqueCount > 12) {
        return { valid: false, message: `Too many facet values (${uniqueCount}). Please filter to 12 or fewer.` };
      }
    }
  }
