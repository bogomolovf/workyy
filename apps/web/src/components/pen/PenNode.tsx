...
export function PenNode({
  ...
  selected,
  dragging,
}: NodeProps<PenNodeType>) {
  // Убеждаемся, что width и height - это числа
  const nodeWidth = typeof width === "number" ? width : typeof width === "string" ? parseFloat(width) : data.initialSize.width;
  const nodeHeight = typeof height === "number" ? height : typeof height === "string" ? parseFloat(height) : data.initialSize.height;
