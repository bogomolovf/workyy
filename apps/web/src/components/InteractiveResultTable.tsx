...
export type InteractiveResultTableProps = {
  ...
  onLoadAll?: () => void;
  maxHeight?: number;
  /** Called when the user scrolls near the bottom to load the next batch */
  onLoadMore?: () => void;
  /** Number of rows loaded so far (for paginated display) */
  loadedCount?: number;
  /** Whether a batch load is in progress */
...
function InteractiveResultTableInner({
  ...
  // Uses a scroll listener instead of depending on virtualRows to avoid render loops.
  const onLoadMoreRef = useRef(onLoadMore);
  onLoadMoreRef.current = onLoadMore;
  const isLoadingMoreRef = useRef(isLoadingMore);
  isLoadingMoreRef.current = isLoadingMore;
  const rowCountForScroll = sortedRowIndices.length;

  useEffect(() => {
    const el = parentRef.current;
    if (!el) return;
    const handleScroll = () => {
      if (!onLoadMoreRef.current || isLoadingMoreRef.current) return;
      const { scrollTop, scrollHeight, clientHeight } = el;
      const nearBottom = scrollTop + clientHeight >= scrollHeight - ROW_HEIGHT * 5;
      ...
      {rowCount > 0 && (
        <div className="flex items-center justify-between text-xs text-slate-500">
          <span className="flex items-center gap-2">
            {isPreview || (loadedCount != null && loadedCount < displayTotal) ? (
              <>
                Showing {formatCount(loadedCount ?? rowCount)} of {formatCount(displayTotal)} rows
            ...
            {isLoadingMore && <span className="text-slate-400">(loading...)</span>}
          </span>
          {isPreview && onLoadAll && (
            <button
              type="button"
...
export const InteractiveResultTable = memo(InteractiveResultTableInner, (prev, next) => {
  return (
    ...
    prev.loadedCount === next.loadedCount &&
