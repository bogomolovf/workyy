...
function getHandlePosition(
  node: Node,
  handleId: "left" | "top" | "right" | "bottom"
): XYPosition {
...
export function findNearestHandleId(
  ...
  const candidateHandles = DATA_NODE_HANDLE_POSITIONS.filter(
    (h) => h.type === handleType
  );
  ...
  for (const handle of candidateHandles) {
    ...
    const distance = Math.sqrt(
      Math.pow(mousePosition.x - handlePos.x, 2) +
        Math.pow(mousePosition.y - handlePos.y, 2)
    );
 ...
 */
export function getHandlePositionForNode(
  node: Node,
  handleId: string
...
export function resolveConnectionEndpoints(connection: Connection, origin: ConnectionOrigin | null): {
  ...
  if (!sourceId || !targetId || sourceId === targetId) {
    return { sourceId: undefined, targetId: undefined, sourceHandle: undefined, targetHandle: undefined };
  }
