...
/** Close code sent when board is deleted; clients should redirect to board list */
export const BOARD_DELETED_CLOSE_CODE = 4000;
export const BOARD_DELETED_CLOSE_REASON = 'board-deleted';

/**
 * Close all WebSocket connections for a board (e.g. after board deletion).
 * All clients on that board receive the close and can redirect to /.
 */
export function closeAllConnectionsForBoard(
  boardId: string,
  ...
  try {
    ...
    if (!doc?.conns) return;
    let closed = 0;
    // In y-websocket the KEY of conns is the WebSocket, value is Set<number>
    for (const [ws] of doc.conns) {
      try {
        if (ws && typeof ws.close === 'function' && ws.readyState === ws.OPEN) {
          ws.close(BOARD_DELETED_CLOSE_CODE, BOARD_DELETED_CLOSE_REASON);
          closed += 1;
        }
      } catch {
        // individual connection close may fail — skip
      }
    }
    if (log) {
      log.info({ boardId, closed }, 'Closed connections for deleted board');
    }
  } catch (err) {
    if (log) {
      log.info(
        { boardId, error: err instanceof Error ? err.message : String(err) },
        'Error closing connections for deleted board',
      );
    }
  }
}
