...
export async function setupCollaborationWS(
  ...
  try {
    ...
    if (!token) {
      fastify.log.warn({ url: request.url, boardId, cookieName }, 'WebSocket rejected: No auth token in cookies');
      ws.close(1008, 'Authentication required');
      return;
    }
    ...
    try {
      ...
      if (ws.readyState !== ws.OPEN && ws.readyState !== ws.CONNECTING) {
        fastify.log.warn({ readyState: ws.readyState, boardId }, 'WebSocket closed before JWT verification');
        return;
      }
    ...
    try {
      ...
      if (ws.readyState !== ws.OPEN && ws.readyState !== ws.CONNECTING) {
        fastify.log.warn({ readyState: ws.readyState, boardId, userId }, 'WebSocket closed before board access check');
        return;
      }
      ...
      if (ws.readyState !== ws.OPEN && ws.readyState !== ws.CONNECTING) {
        fastify.log.warn({ readyState: ws.readyState, boardId, userId }, 'WebSocket closed during board access check');
        return;
      }
    ...
    try {
      ...
      fastify.log.info({ boardId, userId, readyState: ws.readyState }, 'setupWSConnection called successfully');
    } catch (setupError) {
      fastify.log.error(
        {
