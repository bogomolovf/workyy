...
5. При необходимости добавь свои команды в `.cursor/commands/` (см. `.cursor/README.md`)
...
### Пользовательские команды и агенты

Команда может создавать свои команды и агентов в `.cursor/commands/`:

- **Агентские команды** (`.cursor/commands/agents/`) - активируют специализированных агентов
- **Команды задач** (`.cursor/commands/tasks/`) - выполняют конкретные задачи

После создания команды станут доступны через префикс `/` в чате Cursor (если включен "Import Claude Commands" в настройках).

**Подробнее:** См. `.cursor/README.md` для инструкций по созданию команд.

### Полезные промпты

Сохраняйте эффективные промпты в `.cursor/prompts/` для использования командой.
...
3. **Перед PR**:
   - Используй Cursor для проверки кода
   - Исправь замечания
